...
@Component({
  selector: 'app-units-create-lead',
  standalone: true,
  imports: [TabsModule, FormsModule, ReactiveFormsModule, CommonModule],
  templateUrl: './units-create-lead.component.html',
  styleUrl: './units-create-lead.component.scss'
})
export class UnitsCreateLeadComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.leadservice.createLead(this.realestateLeadForm.value).subscribe(
      (res: any) => {
        ...
        console.log(res);
        this.showSuccess();
        this.router.navigate(['/real-estate/units-leads']);
      },
      (error) => {
        this.showError();
      }
