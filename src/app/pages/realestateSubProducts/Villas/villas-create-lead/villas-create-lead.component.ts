...
@Component({
  selector: 'app-villas-create-lead',
  standalone: true,
  imports: [TabsModule, FormsModule, ReactiveFormsModule, CommonModule],
  templateUrl: './villas-create-lead.component.html',
  styleUrl: './villas-create-lead.component.scss'
})
export class VillasCreateLeadComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.leadservice.getPicklist().subscribe((data: any) => {
      ...
      if (this.verticalData1.length > 0) {
        this.lookupConstants = this.verticalData1[0].lookupConstants.filter((item:any)=>item.constantName ==='Apartments');
  ...
  onSubmit() {
    ...
    this.leadservice.createLead(this.realestateLeadForm.value).subscribe(
      (res: any) => {
        ...
        // Success message
        this.showSuccess();
        this.router.navigate(['/real-estate/villas-leads']);
      },
      (error) => {
        this.showError();
      }
