...
@Component({
  selector: 'app-commercial-space-create-lead',
  standalone: true,
  imports: [TabsModule, FormsModule, ReactiveFormsModule, CommonModule],
  templateUrl: './commercial-space-create-lead.component.html',
  styleUrl: './commercial-space-create-lead.component.scss'
})
export class CommercialSpaceCreateLeadComponent {
  ...
  onSubmit() {
    ...
    this.leadservice.createLead(this.realestateLeadForm.value).subscribe(
      (res: any) => {
        ...
        // Success message
        this.showSuccess();
        this.router.navigate(['/real-estate/commercial-space-leads']);
      },
      (error) => {
        this.showError();
      }
