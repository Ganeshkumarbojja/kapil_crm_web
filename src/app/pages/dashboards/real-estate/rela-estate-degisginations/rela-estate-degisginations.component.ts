...
import { UpdatedesignationComponent } from 'src/app/chits/designationlist/updatedesignation/updatedesignation.component';
import { MastersService } from 'src/app/chits/Services/masters.service';
import { RealEsateUpdateDesiginationsComponent } from './real-esate-update-desiginations/real-esate-update-desiginations.component';

@Component({
  selector: 'app-rela-estate-degisginations',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, OrderByPipe, PaginationModule],
  templateUrl: './rela-estate-degisginations.component.html',
  styleUrl: './rela-estate-degisginations.component.scss'
})
export class RelaEstateDegisginationsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.masterService.getRealEstateDesignationListData().subscribe((data: any) => {
  ...
  deleteDesignation() {
    this.masterService.deleterealestaeDesignationData(this.selectedId).subscribe((data) => {
      console.log(data);
      this.display2 = "none";
      this.ngOnInit();
    })
  ...
  onSubmit() {
    ...
    this.masterService.createRealEstateDesignationList(this.designationForm.value).subscribe((data: any) => {
