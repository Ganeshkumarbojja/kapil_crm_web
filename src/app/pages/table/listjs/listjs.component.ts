...
export class ListjsComponent {
  ...
  saveListJs() {
    if (this.listJsForm.valid) {
      if (this.listJsForm.get('ids')?.value) {
        ...
        const reportingTo = this.listJsForm.get('reportingTo')?.value;
        ListJs.push({
  ...
  */
  editModal(id: any) {
    ...
    this.listJsForm.controls['name'].setValue(listData.name);
    this.listJsForm.controls['code'].setValue(listData.code);
    this.listJsForm.controls['reportingTo'].setValue(listData.reportingTo);
